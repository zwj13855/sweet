...
namespace SweetSugar.Scripts.Editor
{
    public class PostImporting : AssetPostprocessor
    {
        ...
        private static void CheckTargets()
        {
            ...
            if (t == null || t.targets == null || t.targets.FirstOrDefault() == null || t.targets.FirstOrDefault()?.defaultSprites == null || t.targets.FirstOrDefault().defaultSprites.Length == 0)
            {
                EditorUtility.DisplayDialog("Restart Unity", "Please restart Unity to initialize new targets", "Ok");
            }
        }
